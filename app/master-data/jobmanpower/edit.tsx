...
export function meta() {
    return [
        { title: "แก้ไข Job Manpower - JobYam Admin" },
        { name: "description", content: "Edit Job Manpower entry" },
    ];
}

export default function EditJobManpower() {
    ...
    useEffect(() => {
        const fetchJobManpower = async () => {
            try {
                const result = await api.get<JobManpower>(`jobmanpower/${id}`);

                if (result.code !== 200 || !result.data) {
                    throw new Error(result.status || "Failed to fetch job manpower");
                }

                // Access the data property which contains the JobManpower object
                const jobManpower = result.data;
                console.log(jobManpower);
                ...
                setFormData({
                    ...
                    certificate: jobManpower.certificate || '',
                    address: jobManpower.address || '',
                    location: jobManpower.location || '',
                    status: jobManpower.status !== undefined ? jobManpower.status : true
                });
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await api.put<ApiResponse<JobManpower>>(`jobmanpower/${id}`, formDataToSend);

            if (result.code !== 200) {
                throw new Error(
                    result.status ||
                    "Failed to update job manpower"
                );
            }
    ...
    return (
        <div className="min-h-screen p-6">
            <div className="max-w-3xl mx-auto">
                <h1 className="text-2xl font-semibold text-gray-900 mb-6">แก้ไข Job Manpower</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 bg-white shadow rounded-lg p-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                            หนังสือรับรอง {formData.certificate && <span>(มีไฟล์อยู่แล้ว)</span>}
                        </label>
                        <input
                            type="file"
                            accept=".pdf"
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            {formData.certificate
                                ? 'อัพโหลดไฟล์ใหม่เฉพาะเมื่อต้องการเปลี่ยน (รองรับไฟล์ PDF เท่านั้น)'
