...
export default function UsersPage() {
  ...
  const fetchUsers = async (forceRefresh = false) => {
    try {
      ...
      const user = JSON.parse(localStorage.getItem('jobyamUserAdmin') || '{}');
      const role = user.role.name || '';
      const type = role.toLowerCase() === 'modulator' ? 'modulator' : 'admin';

      // Build query parameters for API request
      const offset = (currentPage - 1) * pageSize;
      let queryParams = `type=${type}&limit=${pageSize}&offset=${offset}`;
    ...
    <div className="min-h-screen p-6">
      ...
      <div className="w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-[#0038A8]">{language === 'en' ? 'Manage Admin' : 'จัดการผู้ดูแลระบบ'}</h1>
            <p className="text-sm text-[#0038A8] mt-1">{language === 'en' ? 'All users admin in the system' : 'รายการผู้ดูแลระบบทั้งหมดในระบบ'}</p>
          </div>
          <div className="flex gap-2 mt-3 sm:mt-0">
            <Link
              to="/manageadmin/create"
              className="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-md text-sm font-medium text-white bg-[#0038A8] transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0038A8]/50 cursor-pointer"
            >
              <svg className="-ml-1 mr-2 h-5 w-5 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
              </svg>
              <span className="font-semibold">{language === 'en' ? 'Add User' : 'เพิ่มผู้ใช้'}</span>
            </Link>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="w-full md:w-64">
              ...
              <div className="relative">
                ...
                >
                  <option value="">{language === 'en' ? 'All Roles' : 'ทุกบทบาท'}</option>
                  {roles.map(role => (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    >
                      <div className="flex items-center">
                        <span>{language === 'en' ? 'Role' : 'บทบาท'}</span>
                        {sortBy === 'role' && (
                          <svg className="ml-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={sortOrder === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'} />
                          </svg>
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          {user.role.name !== 'User' ? (
                            <Link
                              to={`/manageadmin/${user.id}/edit`}
                              className="text-blue-600 cursor-pointer hover:text-blue-900 px-2 py-1 rounded hover:bg-blue-50"
                            >
                          ...
                          <button
                            ...
                            // hidden={userInfo?.role?.name.toLowerCase() === 'modulator'}
